//! peelfs — userspace virtio-mmio block driver on peel disk (1.16).
//!
...
fn setup_queue(mmio: usize, queue_pa: usize, data_pa: usize, irq_slot: isize) -> Option<Disk> {
    ...
    if max < QUEUE_NUM as u32 {
        let _ = sys::debug_write("peelfs: queue too small\n");
        return None;
    }
...
fn rw_sector(disk: &mut Disk, sector: u64, buf: &mut [u8], write: bool) -> bool {
    ...
    if waited {
        let _ = sys::debug_write("peelfs: irq wait ok\n");
    }
...
pub extern "C" fn main() {
    let _ = sys::debug_write("peelfs: probe start\n");

    let Some((idx, mmio_slot)) = find_second_blk() else {
        let _ = sys::debug_write("peelfs: no second block device\n");
        loop {
            let _ = sys::yield_now();
        }
    };

    let _ = sys::debug_write("peelfs: claim idx=");
    write_dec(idx as u32);
    let _ = sys::debug_write("\n");

    let irq_slot = sys::irq_virtio(idx);
    if irq_slot >= 0 {
        let _ = sys::debug_write("peelfs: irq cap\n");
    } else {
        let _ = sys::debug_write("peelfs: irq mint failed\n");
    }

    let qslot = sys::frame_alloc_n(2);
    let dslot = sys::frame_alloc();
    if qslot < 0 || dslot < 0 {
        let _ = sys::debug_write("peelfs: dma alloc failed\n");
        loop {
            let _ = sys::yield_now();
        }
    }
    ...
    if queue_pa < 0 || data_pa < 0 {
        let _ = sys::debug_write("peelfs: frame_phys failed\n");
        loop {
            let _ = sys::yield_now();
        }
    }
    ...
    {
        let _ = sys::debug_write("peelfs: dma map failed\n");
        loop {
            let _ = sys::yield_now();
        }
    }
    ...
    let _ = sys::debug_write("peelfs: ready sectors=");
    write_dec(disk.capacity as u32);
    let _ = sys::debug_write("\n");

    let mut buf = [0u8; SECTOR_SIZE];
    let magic = b"DeepRoot peelfs 1.17\n";
    for (i, b) in magic.iter().enumerate() {
        buf[i] = *b;
    }
    /* Sector 2 — peelfs teaching store (virtioblk demo used sector 1). */
    if !rw_sector(&mut disk, 2, &mut buf, true) {
        let _ = sys::debug_write("peelfs: write failed\n");
        loop {
            let _ = sys::yield_now();
        }
    }
    let _ = sys::debug_write("peelfs: put ok\n");
    buf.fill(0);
    if !rw_sector(&mut disk, 2, &mut buf, false) {
        let _ = sys::debug_write("peelfs: read failed\n");
        loop {
            let _ = sys::yield_now();
        }
    }
    ...
    if ok {
        let _ = sys::debug_write("peelfs: get ok\n");
        let _ = sys::debug_write("peelfs: server online\n");
    } else {
        let _ = sys::debug_write("peelfs: rw mismatch\n");
        loop {
            let _ = sys::yield_now();
        }
    }

    /* Stay up as a named FS-policy server (1.17 peel). */
    const PEELFS_BADGE: u64 = 0xD018;
    loop {
        let label = sys::ipc_recv(PEELFS_BADGE);
        if label < 0 {
            let _ = sys::yield_now();
            continue;
        }
        let _ = sys::debug_write("peelfs: ipc ok\n");
        let _ = sys::ipc_reply(PEELFS_BADGE, 0x5046, 1); /* 'PF' */
        let _ = sys::yield_now();
    }
}
...
fn panic(_: &core::panic::PanicInfo) -> ! {
    let _ = sys::debug_write("peelfs: PANIC\n");
    sys::exit(1);
}
