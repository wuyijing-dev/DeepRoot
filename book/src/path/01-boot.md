...
- `fdt::probe`：解析自有 DTB（1.5+）  
- `smp::boot_secondaries`：HSM 拉二级核（1.7）  
...
额外建议：先别改代码，直接用脚本给的 smoke 目标“对照验收”。`scripts/run-qemu.sh --smoke` 里会检查一组关键字符串（适用于当前 `VERSION`，如 **1.7.0**）：

`DeepRoot microkernel 1.7.0` / `fdt: model "DeepRoot QEMU virt"` / `fdt: board deeproot,qemu-virt` / `fdt: cpu count=2` / `smp: 2 hart(s) online` / `smp: secondary hart=` / `virtio-blk: ready` / `block: virtio-blk ready` / `DRFS` / `canopy ready` / `ping: pong` / `hello: spawned ELF says hi` / `shell: DeepRoot shell ready` / `init: handing off to shell`。
