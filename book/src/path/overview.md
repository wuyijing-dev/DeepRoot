...
| 段 | 含义 | 初学者怎么用 |
|---|---|---|
| MAJOR | ABI 大断裂或平台跃迁（2.0 = 集成发布，不赶） | 文档基线在 **1.x**，当前推荐 **v1.11.0** |
...
请固定 **`v1.11.0`**（或页首选择器打开对应冻结快照），否则日志和截图对不上。

## 4. 一句话心态

先求「跑起来、看见现象」，再求「为什么这样设计」。  
1.11 起根路径写入可持久；显示与 Wayland 启发路径见 `VERSION` / [下一步](../appendix/next.md)。
